...
class DiSSCoFDO(FDO):
    ...
    @staticmethod
    def get_fdo_profile():
        """Returns the FDO profile definition."""

        # Open the JSON file that contains the FDO type definition
        with open('/data/dissco_fdo_profile.json') as json_file:
            return json.load(json_file)

    @staticmethod
    def get_fdo_record():
        """Returns the FDO Record for a specific FDO, given its PID"""

        # Open the URL that contains the definition for the FDO metadata attributes
        with open('data/dissco_fdo_profile.json') as json_file
